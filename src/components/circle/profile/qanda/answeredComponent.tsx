...
import { getAnsweredQuestions } from "../../../../redux/actions/profileActions";
import { DataTable } from "react-native-paper";
import QuestionCard from "../../common/questionCard";

const PAGE_SIZE: number = 20;
const AnsweredComponent = (props: any) => {
  const dispatch = useDispatch();

  const { answeredQuestions } = useSelector((state: any) => state.profile);
  ...
  useEffect(() => {
    if (answeredQuestions?.data) {
      const _items = answeredQuestions.data;
      const total_items = answeredQuestions?.paginator?.total_count;
  ...
  return (
    <View style={styles.container}>
      {totalItems ? (
        <Text style={styles.count}>{totalItems} Questions answered</Text>
      ) : null}
      <FlatList
